...
const questions = [
    { q: "당신의 연령대는 어떻게 되시나요?", options: [{ text: "20대 - 30대 (공격적 투자 가능)", score: 5 }, { text: "40대 - 50대 (안정적 성장 필요)", score: 3 }, { text: "60대 이상 (자산 보존 우선)", score: 1 }] },
    { q: "투자할 수 있는 기간은 어느 정도인가요?", options: [{ text: "10년 이상 장기 투자", score: 5 }, { text: "3년 ~ 5년 중기 투자", score: 3 }, { text: "1년 이내 단기 투자", score: 1 }] },
    { q: "투자 경험이 얼마나 되시나요?", options: [{ text: "주식, 파생상품 등 고위험 자산 유경험자", score: 5 }, { text: "펀드, ETF 위주의 중위험 자산 유경험자", score: 3 }, { text: "예적금 위주의 안전 자산 선호", score: 1 }] },
    { q: "자산의 20%가 하락했다면 어떻게 행동하시겠나요?", options: [{ text: "오히려 저가 매수의 기회로 삼고 더 산다", score: 5 }, { text: "계획대로 비중을 유지하며 지켜본다", score: 3 }, { text: "불안해서 즉시 매도하여 손실을 확정한다", score: 1 }] },
    { q: "기대 수익률과 감수할 위험 중 무엇이 중요한가요?", options: [{ text: "손실 위험이 커도 고수익이 중요하다", score: 5 }, { text: "적당한 수익과 적당한 위험이 좋다", score: 3 }, { text: "수익이 낮아도 원금 보존이 최우선이다", score: 1 }] },
    { q: "수입의 얼마를 저축/투자하시나요?", options: [{ text: "50% 이상 (여유 자금 풍부)", score: 5 }, { text: "20% ~ 40% (보통)", score: 3 }, { text: "10% 이하 (지출이 많음)", score: 1 }] },
    { q: "투자 목적이 무엇인가요?", options: [{ text: "자산의 공격적인 증식 (파이어족)", score: 5 }, { text: "주택 자금 등 특정 목적의 목돈 마련", score: 3 }, { text: "안정적인 노후 생활 자금", score: 1 }] }
];
...
function renderQuestion() {
    ...
    qData.options.forEach((opt, index) => {
        ...
        btn.className = "w-full p-6 text-left rounded-3xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 hover:border-blue-500 hover:shadow-lg transition-all group";
        btn.innerHTML = `<div class="flex items-center gap-4"><div class="w-10 h-10 rounded-2xl bg-white dark:bg-slate-800 text-slate-400 font-bold flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors shadow-sm">${String.fromCharCode(65 + index)}</div><span class="font-bold text-lg text-slate-700 dark:text-slate-200 group-hover:text-blue-600 transition-colors">${opt.text}</span></div>`;
        btn.onclick = () => window.selectOption(opt.score);
        optionsContainer.appendChild(btn);
    });
}

window.selectOption = async function(score) {
    totalScore += score;
    currentQuestion++;
