...
- **기능:** 전체 단계 데이터 종합 표시 (투자 청사진 요약).
- **연결:** `/api/price` (실시간 시장 지수).
...
4. `Step 7` (재무 요약 및 청사진 확인)
5. `Step 8` (실전 자산 구축 및 관리)
...
7. **도메인 흐름과 대시보드 정합성:** 실전 자산이 등록되는 8단계 이전인 7단계에서 실전 자산 차트를 보여주는 논리적 오류. ➔ **해결:** 7단계를 **'재무 청사진(Blueprint)'** 확인 단계로 재정의하여, 1~6단계에서 수립한 **타겟 비중(Target Weights)**과 목표치를 요약해서 보여주도록 기획 수정.
